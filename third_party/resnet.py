...
class ResNet(nn.Module):
    def __init__(
        ...
        self._pyramid_channels = [self.inplanes]
        self.groups = groups
        ...
        self._pyramid_channels.append(64 * block.expansion)
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0]
        )
        self._pyramid_channels.append(128 * block.expansion)
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1]
        )
        self._pyramid_channels.append(256 * block.expansion)
    ...
    def forward_pyramids(self, x: torch.Tensor) -> torch.Tensor:
        levels = collections.OrderedDict()
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        level1 = self.maxpool(x)
        level2 = self.layer1(level1)
